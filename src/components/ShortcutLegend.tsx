...
export function ShortcutLegend() {
  ...
  return (
    <div className="fixed top-4 right-4 z-30 hidden lg:block">
      {open ? (
        <div className={`px-3 py-2 text-xs space-y-1 backdrop-blur-sm ${s(theme, "tooltip")}`}>
          <button
          ...
          {shortcutEntries.map((shortcut) => (
            <div key={shortcut.key} className="flex items-center justify-between gap-4">
              ...
              <kbd className={`px-1.5 py-0.5 text-[10px] font-mono leading-none ${s(theme, "kbd")}`}>
                {shortcut.label}
              </kbd>
            </div>
          ))}
        ...
        <button
          ...
          className={`h-8 w-8 flex items-center justify-center cursor-pointer backdrop-blur-sm ${s(theme, "tooltip")}`}
